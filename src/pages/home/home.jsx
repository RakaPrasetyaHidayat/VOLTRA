...
function Home() {
  ...
  const filteredTasks = tasks.filter((task) => {
    const matchStatus = filterCriteria.status === "All" || task.status === filterCriteria.status;
    const matchPriority = filterCriteria.priority === "All" || task.priority === filterCriteria.priority;
  ...
  const handleSubmitCreate = async () => {
    ...
    if (data) {
      await fetchTasks();
      setShowCreate(false);
      setForm({ projectId: 1, teamId: "", title: "", deadline: "", assignedUserId: "", status: "To Do", priority: "Low" });
    }
  };
  ...
  const handleDelete = async (task) => {
    ...
    await safeFetch(`${API({ method: "DELETE" })}/tasks/${id}`, { method: "DELETE" });
    fetchTasks();
  };
  ...
  function formatDate(date) {
    ...
    return isNaN(d) ? "-" : d.toLocaleDateString("id-ID", { day: "numeric", month: "short" });
  }
        ...
        <div className={styles.project}>
          ...
          <div className={styles.projectContent}>
            <div className={styles.card}>
              <span><img src={todoList} width="25" alt="" /> <h3>To-Do List</h3></span>
              <div className={styles.cardItems}><p>{tasks.length}</p><p><b>Tasks</b></p></div>
            </div>
            <div className={styles.card}>
              <span><img src={clock} width="25" alt="" /> <h3>Project Updates</h3></span>
              <div className={styles.cardItems}><p>-</p><p><b>Ongoing</b></p></div>
            </div>
            <div className={styles.card}>
              <span><img src={quicknotes} width="25" alt="" /> <h3>Quick Notes</h3></span>
              <div className={styles.cardItems}><p>-</p><p><b>Notes</b></p></div>
            </div>
          </div>
        </div>
        ...
        <div className={styles.updates}>
          <div className={styles.utilities}>
            ...
            <div style={{ position: 'relative' }}>
              <span onClick={() => setShowFilter(!showFilter)} style={{ cursor: 'pointer' }}>
              ...
              {showFilter && (
                <div className={styles.filterMenu}>
                  ...
                  <button onClick={() => { setFilterCriteria({ status: "All", priority: "All" }); setShowFilter(false); }}>Reset</button>
                </div>
              )}
            </div>

            <span onClick={() => setShowCreate(true)} style={{ cursor: 'pointer' }}>
              <img src={whitePlus} alt="" />New Task
            </span>
          </div>

          {/* TABLE - MENGGUNAKAN filteredTasks */}
          <div className={styles.updatesContent} style={{ gridTemplateColumns: "2fr 1.2fr 1fr 1fr 1.2fr 1fr" }}>
            <div><b>Title</b></div>
            ...
            {filteredTasks.map((task) => (
              <div key={getId(task)} style={{ display: "contents" }} onDoubleClick={() => handleDelete(task)}>
                <div className={styles.firstColumn}><b>{task.title}</b></div>
                <div className={styles.updatesMainContent}>{formatDate(task.deadline)}</div>
                <div className={styles.updatesMainContent}>{task.teamId || "-"}</div>
                <div className={styles.updatesMainContent}>
                  <img src={account} width="14" alt="" style={{ marginRight: '4px' }} />
                  {task.assignedUserId || "-"}
                </div>
                <div className={styles.updatesMainContent}>
                  <select
                ...
                <div className={styles.updatesMainContent}>
                  <div className={`${getPriorityClass(task.priority)} ${styles.label}`}>
                    {task.priority}
                  </div>
                </div>
              </div>
          ...
          {showCreate && (
            <div className={styles.popupOverlay}>
              <div className={styles.popup} style={{ display: 'flex', flexDirection: 'column', gap: '12px', minWidth: '350px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  <div className={styles.formGroup} style={{ flex: 1 }}>
                    ...
                    <input placeholder="0" type="number" value={form.teamId} onChange={(e) => setForm({ ...form, teamId: e.target.value })} />
                  </div>
                  <div className={styles.formGroup} style={{ flex: 1 }}>
                    <label><b>Assignee ID</b></label>
                    <input placeholder="0" type="number" value={form.assignedUserId} onChange={(e) => setForm({ ...form, assignedUserId: e.target.value })} />
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '10px' }}>
                  <div className={styles.formGroup} style={{ flex: 1 }}>
                ...
                <div className={styles.buttonContainer}>
                  <button className={styles.saveButton} onClick={handleSubmitCreate}>Create</button>
